...
function jwtRole(jwt) {
  try { const payload = JSON.parse(Buffer.from(jwt.split(".")[1], "base64").toString("utf8")); return payload?.role || "unknown"; }
...
const previewOrigins = String(process.env.APP_PREVIEW_ORIGINS || "").split(",").map((x) => x.trim()).filter(Boolean);
const allowedOrigins = new Set([process.env.APP_PUBLIC_URL,"http://localhost:5173","https://shiftcensus.com","https://www.shiftcensus.com","https://app.shiftcensus.com",...previewOrigins].filter(Boolean));
const corsOptions = {
  origin: (origin, cb) => { if (!origin) return cb(null, true); if (allowedOrigins.has(origin)) return cb(null, true); return cb(new Error(`CORS blocked for origin: ${origin}`), false); },
...
app.use("/api/census/move-swap", requireAuth, requireOrg, requireCensusAccess, require("./routes/censusMoveAtomic"));
...
app.use("/api/shift-requests", require("./routes/pickupWorkflow"));
app.use("/api/shift-requests", require("./routes/requestRevocations"));
app.use("/api/shift-requests", require("./routes/requestQueueFast"));
app.use("/api/shift-requests", require("./routes/requestDecisionWorkflow"));
...
app.get("/health/ready", async (_req, res) => {
  try {
    const dbCheck = supabaseAdmin.from("orgs").select("id").limit(1);
    const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error("Database readiness timeout")), 2500));
