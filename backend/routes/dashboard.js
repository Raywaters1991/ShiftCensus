...
const settingsQuery=supabaseAdmin.from("org_settings").select("timezone,lunch_break_minutes").eq("org_id",orgId).maybeSingle();
...
return res.json({date,timezone,lunch_break_minutes:lunchBreakMinutes,configured_shift_types:configuredShiftTypes,census:{occupied,leave,empty,total},shifts,active_shifts:activeShifts,staff:staffResult.data||[],assignments:assignmentResult.data||[],units:unitResult.data||[],meta:{server_ms:Date.now()-started}});
