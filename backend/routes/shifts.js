...
function computeShiftTimes(shift_date, setting, timezone) {
  ...
  if (setting.end_local < setting.start_local) { const d = new Date(endUtc); d.setUTCDate(d.getUTCDate() + 1); endUtc = d.toISOString(); }
...
router.get("/", async (req, res) => {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("shifts").select("*").eq("org_code", orgCode).order("start_time", { ascending: true });
...
router.post("/", async (req, res) => {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("shifts").insert([{ staff_id, role, unit, assignment_number, shift_date, start_local: setting.start_local, end_local: setting.end_local, start_time: startUtc, end_time: endUtc, timezone: facilityTimezone, org_code: orgCode }]).select();
    if (error) throw error;
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const { startUtc, endUtc } = computeShiftTimes(shift_date, setting, facilityTimezone);
    const { data, error } = await supabaseAdmin.from("shifts").update({ staff_id, role, unit, assignment_number, shift_date, start_local: setting.start_local, end_local: setting.end_local, start_time: startUtc, end_time: endUtc, timezone: facilityTimezone }).eq("id", id).eq("org_code", orgCode).select();
    if (error) throw error;
    res.json(data?.[0] || null);
