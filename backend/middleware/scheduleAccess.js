...
async function requireScheduleAccess(req, res, next) {
  try {
    ...
    if (!userId || !orgId) {
      return res.status(401).json({ error: "Missing auth or org context" });
    }
    ...
    if (write && !membership.can_schedule_write) {
      return res.status(403).json({ error: "Schedule write permission required" });
    }

    if (!write && !membership.can_schedule_read && !membership.can_schedule_write) {
      return res.status(403).json({ error: "Schedule read permission required" });
    }
