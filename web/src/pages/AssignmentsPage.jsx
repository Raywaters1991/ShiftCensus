...
export default function AssignmentsPage(){
  ...
  async function load(){
    ...
    try{
      const [s,p,u,a]=await Promise.all([
        api.get("/shifts"),api.get("/staff"),api.get("/units"),api.get(`/shift-assignments?date=${encodeURIComponent(date)}`)
      ]);
      setShifts((Array.isArray(s)?s:[]).filter(x=>x.shift_date===date));
      setStaff(Array.isArray(p)?p:[]);setUnits(Array.isArray(u)?u:[]);setAssigned(Array.isArray(a)?a:[]);
  ...
  async function setUnit(shift,unitName){
    ...
    try{
      if(!unitName){await api.delete(`/shift-assignments/${shift.id}`);}else{
        await api.put(`/shift-assignments/${shift.id}`,{unit_id:unit?.id||null,unit:unitName,assignment_number:null});
      }
      await load();
  ...
  return <div style={{padding:24,color:"var(--text)",maxWidth:1100,margin:"0 auto"}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap",marginBottom:20}}>
      <div><h1 style={{margin:0}}>Assignments</h1><div style={{opacity:.65,marginTop:4}}>Everyone scheduled for the day appears here. Assign each working staff member to a unit.</div></div>
