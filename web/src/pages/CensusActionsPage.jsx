    ...
    {target&&canWrite&&<div style={overlay} onMouseDown={()=>!busy&&setTarget(null)}><div style={modal} onMouseDown={e=>e.stopPropagation()}>
      {mode==="actions"&&<>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10,marginTop:18}}>
          <button style={btn} onClick={startEdit} disabled={busy}>Edit Resident</button>
          {normStatus(target.status)==="leave"?<button style={primary} onClick={()=>setLeave("occupied")} disabled={busy}>Return from Leave</button>:<button style={btn} onClick={()=>setLeave("leave")} disabled={busy}>Send on Leave</button>}
          <button style={btn} onClick={()=>setMode("move")} disabled={busy}>Move / Swap</button>
          <button style={danger} onClick={discharge} disabled={busy}>Discharge</button>
        </div>
        <div style={{display:"flex",justifyContent:"flex-end",marginTop:18}}><button style={btn} onClick={()=>setTarget(null)} disabled={busy}>Close</button></div>
      </>}
      {mode==="move"&&<>
        <div style={{fontSize:22,fontWeight:900}}>Move / Swap Resident</div>
        <div style={{opacity:.72,marginTop:4}}>From bed {bedKey(target)}</div>
        <div style={{marginTop:18}}><div style={{fontWeight:800,marginBottom:7}}>Destination Bed</div><select style={input} value={toId} onChange={e=>setToId(e.target.value)}><option value="">Select…</option>{destinations.map(r=><option key={r.id} value={r.id} disabled={String(r.id)===String(target.id)}>{bedKey(r)} • {normStatus(r.status)==="empty"?"Empty":normStatus(r.status)==="leave"?"On Leave":"Occupied"}</option>)}</select></div>
        <div style={{display:"flex",justifyContent:"flex-end",gap:10,marginTop:18}}><button style={btn} onClick={()=>setMode("actions")} disabled={busy}>Back</button><button style={primary} onClick={move} disabled={busy||!toId}>{busy?"Working…":"Confirm Move / Swap"}</button></div>
      </>}
      {mode==="edit"&&draft&&<>
        <div style={{fontSize:22,fontWeight:900}}>Edit Resident</div><div style={{opacity:.72,marginTop:4}}>Bed {bedKey(target)}</div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:12,marginTop:18}}>
          <label>Payer<select style={input} value={draft.payer_source||""} onChange={e=>setDraft({...draft,payer_source:e.target.value})}><option value="">Select…</option>{PAYER_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></label>
          <label>Care Type<select style={input} value={draft.care_type||""} onChange={e=>setDraft({...draft,care_type:e.target.value,expected_discharge:allowsDc(e.target.value)?draft.expected_discharge:""})}><option value="">Select…</option>{CARE_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></label>
          <label>Gender<select style={input} value={draft.patient_gender||"Unknown"} onChange={e=>setDraft({...draft,patient_gender:e.target.value})}>{GENDER_OPTIONS.map(x=><option key={x}>{x}</option>)}</select></label>
          <label>Name / Label<input style={input} value={draft.patient_label||""} onChange={e=>setDraft({...draft,patient_label:e.target.value})}/></label>
          <label>Admit Date<input type="date" style={input} value={draft.admit_date||""} onChange={e=>setDraft({...draft,admit_date:e.target.value})}/></label>
          {allowsDc(draft.care_type)&&<label>Expected Discharge<input type="date" style={input} value={draft.expected_discharge||""} onChange={e=>setDraft({...draft,expected_discharge:e.target.value})}/></label>}
          <label style={{gridColumn:"1/-1"}}>Note<input style={input} value={draft.private_pay_note||""} onChange={e=>setDraft({...draft,private_pay_note:e.target.value})}/></label>
        </div>
        <div style={{display:"flex",justifyContent:"flex-end",gap:10,marginTop:18}}><button style={btn} onClick={()=>setMode("actions")} disabled={busy}>Back</button><button style={primary} onClick={saveEdit} disabled={busy}>{busy?"Saving…":"Save Changes"}</button></div>
      </>}
    </div></div>}
  </div>;
}
