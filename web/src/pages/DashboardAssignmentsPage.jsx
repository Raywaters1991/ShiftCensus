...
export default function DashboardAssignmentsPage(){
  ...
  const assignmentShift=groups[current]?.length?current:(groups.Day?.length?"Day":groups.Night?.length?"Night":"Evening");
  ...
  return <div style={{padding:32,color:"var(--text)",position:"relative"}}>
    ...
    <div style={{display:"flex",justifyContent:"center",marginBottom:16}}>
      <button onClick={()=>setShowAssignments(true)} style={{padding:"12px 22px",borderRadius:999,border:"1px solid var(--border)",background:"#2563eb",color:"white",fontWeight:950,fontSize:15,cursor:"pointer",boxShadow:"0 8px 22px rgba(37,99,235,.22)"}}>View {assignmentShift} Shift Assignments</button>
    </div>
    ...
    {showAssignments&&<AssignmentOverlay
      shiftLabel={assignmentShift}
      rows={groups[assignmentShift]||[]}
      staffById={staffById}
...
function ShiftCard({label,active,rows,staffById,asgByShift,patientDays}){
  ...
  const line=s=>{const p=staffById[String(s.staff_id)]||{};const a=asgByShift[String(s.id)]||{};const role=String(p.role||s.role||"STAFF").toUpperCase();return `${role} — ${a.unit||"Unassigned"}${a.assignment_number?` · Assignment #${a.assignment_number}`:""} — ${fmtHours(shiftHours(s))}`};
...
function AssignmentOverlay({shiftLabel,rows,staffById,asgByShift,secondsLeft,onClose}){
  ...
  return <div style={{position:"fixed",inset:0,zIndex:5000,background:"rgba(3,7,18,.97)",color:"white",padding:"32px",overflowY:"auto"}}>
    <div style={{maxWidth:1100,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:16,flexWrap:"wrap",borderBottom:"1px solid rgba(255,255,255,.15)",paddingBottom:18,marginBottom:22}}>
        <div><div style={{fontSize:13,fontWeight:900,letterSpacing:".12em",textTransform:"uppercase",opacity:.65}}>ShiftCensus Wallboard</div><h1 style={{margin:"5px 0 0",fontSize:34}}>{shiftLabel} Shift Assignments</h1></div>
        <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{fontWeight:900,opacity:.8}}>Returning to census in {secondsLeft}s</div><button onClick={onClose} style={{padding:"10px 16px",borderRadius:10,border:"1px solid rgba(255,255,255,.25)",background:"rgba(255,255,255,.1)",color:"white",fontWeight:900,cursor:"pointer"}}>Close</button></div>
      </div>
      {ordered.length===0?<div style={{padding:32,textAlign:"center",fontSize:22,opacity:.7}}>No staff scheduled for this shift.</div>:<div style={{display:"grid",gap:12}}>{ordered.map(s=>{
        const p=staffById[String(s.staff_id)]||{};const a=asgByShift[String(s.id)]||{};const role=String(p.role||s.role||"STAFF").toUpperCase();
        return <div key={s.id} style={{display:"grid",gridTemplateColumns:"minmax(220px,1.4fr) 110px minmax(180px,1fr) 150px",gap:16,alignItems:"center",padding:"17px 20px",borderRadius:14,border:"1px solid rgba(255,255,255,.14)",background:"rgba(255,255,255,.06)",fontSize:18}}>
          <div style={{fontWeight:950,fontSize:21}}>{p.name||`Staff #${s.staff_id}`}</div>
          <div style={{fontWeight:950}}>{role}</div>
          <div>{a.unit||"Unassigned"}{a.assignment_number?` · Assignment #${a.assignment_number}`:""}</div>
          <div style={{textAlign:"right",fontWeight:900}}>{fmtHours(shiftHours(s))}</div>
        </div>})}</div>}
    </div>
  </div>
}
