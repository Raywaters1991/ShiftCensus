...
function netShiftHours(s,mealMinutes){return Math.max(shiftHours(s)-Math.max(Number(mealMinutes||0),0)/60,0);}
...
function unitHourRows(rows,units,asgByShift,mealMinutes){return units.map(unit=>{const hours=rows.reduce((sum,s)=>{const a=asgByShift[String(s.id)]||{};const sameId=a.unit_id!=null&&String(a.unit_id)===String(unit.id);const sameName=a.unit&&String(a.unit)===String(unit.name);return sum+((sameId||sameName)?netShiftHours(s,mealMinutes):0);},0);return {id:unit.id,name:unit.name,hours};});}
...
export default function DashboardAssignmentsPage(){
  ...
  const staffedShifts=useMemo(()=>shifts.filter(s=>s.staff_id!=null),[shifts]);
  const patientDays=Number(census.occupied||0)+Number(census.leave||0);
  const scheduledHours=useMemo(()=>staffedShifts.reduce((sum,s)=>sum+netShiftHours(s,lunchBreakMinutes),0),[staffedShifts,lunchBreakMinutes]);
  ...
  return <div style={{height:"calc(100dvh - 64px)",minHeight:0,overflow:"hidden",padding:"10px 24px 14px",color:"var(--text)",position:"relative",boxSizing:"border-box"}}>
    ...
    <div style={{maxWidth:1180,height:"100%",margin:"0 auto",display:"grid",gridTemplateRows:"auto auto auto auto minmax(0,1fr)",rowGap:"8px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:`repeat(${Math.min(visible.length,3)},minmax(0,1fr))`,gap:14,width:"100%",minHeight:0,alignItems:"stretch"}}>{visible.map(key=><ShiftCard key={key} label={`${key} Shift`} active={current===key} rows={displayGroups[key]||[]} units={units} staffById={staffById} asgByShift={asgByShift} mealMinutes={lunchBreakMinutes}/>)}</div>
    </div>
...
function ShiftCard({label,active,rows,units,staffById,asgByShift,mealMinutes}){
  ...
  const licensedUnits=unitHourRows(licensed,units,asgByShift,mealMinutes);
  const cnaUnits=unitHourRows(cnas,units,asgByShift,mealMinutes);
...
function RoleGroup({title,rows,staffById,asgByShift}){
  return <div style={{padding:"14px 15px",borderBottom:"1px solid var(--border)"}}><div style={{fontSize:12,fontWeight:950,textTransform:"uppercase",letterSpacing:".08em",opacity:.6,marginBottom:8}}>{title}</div>{rows.length===0?<div style={{fontSize:13,opacity:.55}}>None scheduled.</div>:rows.map(s=>{const p=staffById[String(s.staff_id)]||{},a=asgByShift[String(s.id)]||{},role=String(p.role||s.role||"STAFF").toUpperCase(),unit=a.unit||"Unassigned",unassigned=!a.unit;return <div key={s.id} style={{display:"grid",gridTemplateColumns:"minmax(95px,.8fr) minmax(0,1.6fr) auto",gap:10,alignItems:"center",padding:"10px 0",borderTop:"1px solid rgba(127,127,127,.12)",background:unassigned?"rgba(245,158,11,.08)":"transparent"}}><div style={{fontWeight:950,color:unassigned?"#f59e0b":"inherit",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{unit}</div><div style={{minWidth:0}}><div style={{fontWeight:950,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{p.name||`Staff #${s.staff_id}`}</div><div style={{fontSize:11,opacity:.6,marginTop:2}}>{role}</div></div><div style={{fontSize:12,fontWeight:850,whiteSpace:"nowrap",opacity:.8}}>{s.shift_type==="Custom"?`Custom ${shiftTimeRange(s)}`:shiftTimeRange(s)}</div></div>})}</div>
}
function Stat({label,value,note}){return <div style={{padding:"8px 12px",borderRadius:11,border:"1px solid var(--border)",background:"var(--card-bg,rgba(255,255,255,.08))",textAlign:"center",minWidth:0}}><div style={{fontSize:10,fontWeight:800,opacity:.65,textTransform:"uppercase",letterSpacing:".06em"}}>{label}</div><div style={{fontSize:"clamp(20px,3vh,28px)",fontWeight:950,lineHeight:1.05}}>{value}</div>{note&&<div style={{fontSize:9,opacity:.5,marginTop:2}}>{note}</div>}</div>}
